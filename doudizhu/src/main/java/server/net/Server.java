...
public class Server {
    ...
    private static void runGameFlow() {
        while (true) {
            ...
            ActionResult actionResult = GAME_FLOW.handlePlayerAction(
                    currentRoom,
                    result.getPlayerId(),
                    actionType
            );
    ...
    //用于处理当前阶段 获取当前阶段的状态
    public static Result processingStatus(Result result,GameRoom room){
        if (room.getHighestScore() >=1){
            result.setCurrentStatus(MessageType.ROB_LANDLORD);
        }
